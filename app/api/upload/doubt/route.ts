...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2. Authorize role (All authenticated roles can post doubts/replies)
    if (!["STUDENT", "FACULTY", "ADMIN", "SUPER_ADMIN"].includes(user.role)) {
      return NextResponse.json(
        { message: "Forbidden: Doubt creator permissions required" },
        { status: 403 }
      );
    }
    ...
    // 5. Validate file size (10MB limit for doubts)
    ...
    const allowedExtensions = [
      "png", "jpg", "jpeg", "pdf", "doc", "docx", "zip", "txt"
    ];
    ...
    const storagePath = `${instituteId}/doubts/${uniqueName}`;
    ...
    try {
      ...
      console.error("Failed to record usage snapshot on doubt upload:", snapshotErr);
    }
    ...
    console.error("DOUBT FILE UPLOAD ERROR:", error);
    return NextResponse.json(
